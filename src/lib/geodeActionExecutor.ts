...
The signed contract details have been uploaded to Monday.com → GEODE Payments board under the appropriate state group.
...
function generateOutreachEmailDraft(params: OutreachEmailParams): DraftEmailOptions {
  const { authorName, authorEmail, stateName, stateAbbrev, chapterTitle, chapterNum, senderName } = params;

  const subject = `[GEODE ${stateAbbrev}] Invitation to Contribute - Ch ${chapterNum}: ${chapterTitle}`;

  const body = `Dear ${authorName},

I hope this email finds you well. I'm reaching out on behalf of Project InnerSpace regarding an exciting opportunity to contribute to our GEODE (Geothermal Data for Energy Decisions) initiative.

We are developing the ${stateName} State Geothermal Data Report, and based on your expertise, we would be honored to have you serve as the bylined author for Chapter ${chapterNum}: ${chapterTitle}.

**About the Opportunity:**
- **Project:** GEODE ${stateName} State Geothermal Data Report
- **Chapter:** ${chapterNum} - ${chapterTitle}
- **Role:** Bylined Chapter Author
- **Timeline:** Final report due April 30, 2026

The GEODE reports are DOE-funded publications that help states understand their geothermal potential. Your contribution would provide valuable insights into ${chapterTitle.toLowerCase()} considerations for ${stateName}.

**Attached** is the contributor agreement with full details about the scope, timeline, and compensation. Please review at your convenience.

If you're interested or have any questions, I'd be happy to schedule a brief call to discuss further.

Thank you for considering this opportunity. We believe your expertise would be invaluable to this project.
